                                  ...
                                  CREATE PROCEDURE [3f6f33b5-8eff-4b97-b7bd-a29488b876db].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [3f6f33b5-8eff-4b97-b7bd-a29488b876db].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [3f6f33b5-8eff-4b97-b7bd-a29488b876db].data_input_row (id, name, rowMeta, visualRowConfigId, dimensionId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId,temp.dimensionId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [3f6f33b5-8eff-4b97-b7bd-a29488b876db].' + @TempTableName + ' as temp;';
